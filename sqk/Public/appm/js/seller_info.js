...
function getDetail(id) {
    $.get(c_path + "/getSellerDetail/id/" + id, function (data) {
        if (data.flag == 1) {
            $("#sellerName").html(data.name);
            $("#isRest").html('<b>营业时间：</b>' + data.is_rest);
            $("#address").html('<b>商家地址：</b>' + data.address);
            $("#tel").html('<b>商家电话：</b>' + data.tel);
            //增强体验
            $("#introduction").css('display', 'none');
//            $("#introduction").html(data.introduction.replace(/style="(.)*?"|^\s*|\&nbsp;/gi, ''));
            $("#introduction").html(data.introduction);
            $('#introduction table').css({'width': '100%'});
            $('#introduction table').attr({'border': '1'});
            $('#introduction').html(img_reset($('#introduction').html()));
            if ($('#introduction img').length > 0) {
            ...
            if (data.item.flag == 1) {
                for (var i = 0; i < data.item.data.length; i++) {
                    itemListStr += '<div class="item-list" style="background-image: url(' + res_path + data.item.data[i]['logo_img'] + ');">' +
                            ...
                            '<div class="item-detail font12 fontblack">[' + data.item.data[i]['cat_name'] + ']' + data.item.data[i]['introduction'].replace(/<[^>]+>/gi, '') +
                            '<p class="font11">销量&#12288;' + data.item.data[i]['sold_num'] + data.item.data[i]['quantifier'] + '</p>' +
                            '<p class="fontred m-float-l font16"><b>￥' + data.item.data[i]['price'] + '</b>&nbsp;/' + data.item.data[i]['quantifier'] + '</p><p>' +
                            '<div class="mui-numbox price-numbox" style="margin-left:0px;padding-left:0px;">' +
 ...
 */
function back() {
    window.location.href = "seller_home.html?cat_id=0";
}