    ...
    $.get(c_path + "/getSellerCat", function (data) {
        if (data.flag == 1) {
            ...
            for (var i = 0; i < data.data.length; i++) {
                str += '<li class="mui-table-view-cell mui-media mui-col-xs-2 mui-col-sm-2" style="padding: 0;">' +
                        '<a href="javascript:void(0)" onclick="getBotList(' + type + ',' + data.data[i]['id'] + ',1,0)">' +
                        '<img src="' + public + '/app/img/' + data.data[i]['sys_name'] + '.png" style="width:40px;height:40px;">' +
            ...
            if (data.flag == 1) {
                ...
                for (var i = 0; i < data.data.length; i++) {
                    str += '<li class="mui-table-view-cell" onclick="getSellerDetail(' + data.data[i]['id'] + ')">' +
                            ...
                            '<div class="listcontent" style="padding:10px 5px 0px 10px;height:80px;border-bottom: 1px solid #ddd;border-top: 1px solid #ddd;background-color: #fff;">' +
                            '<div class="listimg" >' +
                            '<img src="' + res_path + data.data[i]['logo_icon'] + '" style="width: 90px;height:60px;">' +
                            '</div>' +
                            '<p class="mui-ellipsis">&#12288;<b class="fontblack">主营分类：</b>' + data.data[i]['item_cat'] + '</p>' +
                            '<p class="mui-ellipsis">&#12288;<b class="fontblack">联系方式：</b>' + data.data[i]['tel'] + '</p>' +
                            '<p class="mui-ellipsis">&#12288;<b class="fontblack">商家地址：</b>' + data.data[i]['address'] + '</p>' +
                            '</div></div>' +
            ...
            if (data.flag == 1) {
                ...
                for (var i = 0; i < data.data.length; i++) {
                    str += '<li class="mui-table-view-cell" onclick="getPromDetail(' + data.data[i]['id'] + ',' + data.data[i]['seller_id'] + ')">' +
                            ...
                            '<img src="' + res_path + data.data[i]['logo_icon'] + '" style="width: 90px;height:60px;">' +
                            '</div>' +
                            '<p class="mui-ellipsis"><b class="fontorder font14">☆&nbsp;' + data.data[i]['title'] + '</b></p>' +
                            '<p class="mui-ellipsis"><font color="#777">' + data.data[i]['ad_time'] + '</font></p>' +
                            '<p class="mui-ellipsis font10">' + data.data[i]['content'].replace(/<[^>]+>/gi, '').substring(0, 60) + '...【详情】</p>' +
                            '</div></div>' +
                            '<div class="listfooter"><span>累计销量：' + data.data[i]['sum_num'] + '&nbsp;件</span></div>' +
                            '</li>';
                }
