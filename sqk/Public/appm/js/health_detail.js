...
function getHealthDetail(id, table_num) {
    ...
    $.get(c_path + "/getHealthDetail/id/" + id + '/table_num/' + table_num, function (data) {
        if (data.flag == 1) {
            $('#title').html('<span>' + data['table_name'] + '</span>&#12288;<span>' + data.data['time'] + '</span>');
            $('#userName').html('<button type="button" class="mui-btn mui-btn-outlined" style="border:0px;"><span class="mui-icon mui-icon-contact" style="font-size:16px;"></span>' + data['realname'] + '&#12288;&#12288;' + data['data']['idcard']) + '</button>';
            $('#time').html();
...
//                血糖
                str = '<div class="cardcontent-icon m-icon-xuetang">' +
                        ...
                        ' <p><span class="fontblack">正常值&nbsp;</span><span class="fontred font16">3.9~6.4</span></p>' +
                        '</div>' +
                        '</div>';
