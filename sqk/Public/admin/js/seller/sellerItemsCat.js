...
//保存通知分类信息
function saveSellerItemsCat(index) {
    $.post(c_path + '/saveSellerItemsCat', {"form_data": $('#save_form').serialize()}, function (result) {
        if (result.code == '500') {
            layer.close(index);
...
function delCatLayer(id){
    layer.confirm('确定要删除此信息嘛？', {
        ...
        $.post(c_path + '/delSellerItemsCat',{'id':id},function(result){
            if(result.code == '500'){
                layer.msg(constants.SUCCESS,{time: 1000},function(){
                    location.reload();
                });
