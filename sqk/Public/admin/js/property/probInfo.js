...
$(function() {
    ...
    $('#dealArrayInfo-btn').click(function(){
        ...
        if($("input[name='rowChecked']:checked").length <= 0){
            layer.msg('请选择批量处理的数据！',{time: 2000});return;
        }else{
...
function dealInfoLayer(isChecked){
    ...
    layer.open({
        type:1,
        title:['问题/诉求处理','font-size:16px;font-weight: bold;color: #2e8ded;'],
        ...
        btn: ['确定', '取消'],
        yes: function (index) {
            dealProbInfo(index,isChecked,$('#is_deal').val(),$('#reply').val());
        },
...
function delInfoLayer(isChecked){
    layer.confirm('确定要删除此险情嘛？', {
        icon:2,
        title:'提示信息',
        btn: ['确定','取消'] //按钮
    }, function(index){
        $.post(c_path + '/delArrProbInfo',{'ids':isChecked},function(result){
            if(result.code == '500'){
...
//处理信息
function dealProbInfo(index,isChecked,is_deal,reply){
    layer.close(index);
    layer.confirm('确定要批量处理险情嘛？', {
        ...
        $.post(c_path + '/dealArrProbInfo',{'ids':isChecked,'is_deal':is_deal,'reply':reply},function(result){
            if(result.code == '500'){
                layer.msg(constants.SUCCESS,{time: 1000},function(){
                    location.reload();
                    window.parent.getPropNum();
