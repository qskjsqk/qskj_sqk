...
//------------------------------------------------------------------------------
    ...
    public function getSellerList($where) {
        $Arr = M('GoodsExchangeRecord')
                ...
                ->where($where)->group('seller_id')->order('exchange_integral desc')->limit(8)
                ->select();
        for ($i = 0; $i < count($Arr); $i++) {
    ...
    public function getSignNumAActivNum($start, $end, $id) {
        ...
        $return['signNum'] = M('ActivSignin')
                ...
                ->count();
        $return['activNum'] = M('ActivInfo')->where('address_id=' . $id)->count();
        dump('----------------');
        return $return;
    }
        ...
        switch ($type) {
            ...
            case 3:
                ...
                switch ($nla) {
                    ...
                    //总榜------------------------------------------------------
                    case 2:
                        $data['topList'] = $this->getCommList('1=1');
                        break;
                }
                break;
        }
